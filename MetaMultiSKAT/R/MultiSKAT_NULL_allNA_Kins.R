MultiSKAT_NULL_allNA_Kins <-
function (y.mat, X,U,D,V_g,V_e,is.fast = TRUE, method = "nlminb") 
{
  ...
  Cov[mis.pheno,] <- 0; Cov[,mis.pheno] <- 0
  Corr[mis.pheno,] <- 0; Corr[,mis.pheno] <- 0
  ...
  V_g[mis.pheno,] <- 0; V_g[,mis.pheno] <- 0
  ...
  for(i in 1:n){
    id<-((i-1)*n.pheno+1):(i*n.pheno)
    V_tilde[id]<-Get_GenInverse(V.item + d[i]*V_g) %*% A[i,]
  ...
  for(i in 1:n){
    id<-((i-1)*n.pheno+1):(i*n.pheno)
    XUVU1X1<-XUVU1X1 + as.matrix(t(U1X[id,])) %*% Get_GenInverse(V.item + d[i]*V_g) %*% as.matrix(U1X[id,])
    
  }
  XUVU1X1_inv<-Get_GenInverse(XUVU1X1)
